...
export type SelectedInscriptionPdfData = {
  header: PdfHeaderDefinition;
  accounts: SelectedInscriptionPdfAccount[];
};

export type SelectedInscriptionPdfAccount = {
  id: string;
  username: string;
  inscriptions: SelectedInscriptionPdfInscription[];
};

export type SelectedInscriptionPdfInscription = {
  id: string;
  responsible: string;
  createdAt: Date;
  participants: SelectedInscriptionPdfParticipant[];
};

export type SelectedInscriptionPdfParticipant = {
  name: string;
  birthDate: string;
  gender: genderType;
};

export class SelectedInscriptionPdfGeneratorUtils {
  public static generateSelectedInscriptionListPdf(
    data: SelectedInscriptionPdfData,
  ): Promise<Buffer> {
    ...
    const content = [
      ...headerContent,
      ...this.buildAccountsContent(data.accounts),
    ];
  ...
  private static buildAccountInscriptions(
    ...
    return inscriptions.map((inscription, inscriptionIndex) => ({
      unbreakable: true,
      margin: [0, inscriptionIndex === 0 ? 0 : 12, 0, 10],
      stack: [
        {
        ...
        this.buildParticipantsTable(inscription.participants),
      ],
    }));
  }

  private static buildParticipantsTable(
    participants: SelectedInscriptionPdfParticipant[],
  ) {
    return {
      table: {
