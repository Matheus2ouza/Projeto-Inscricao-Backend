...
export class PaymentApprovedEmailHandler {
  ...
  private readonly EMAIL_COOLDOWN_KEY = 'payment_approved_email_cooldown';
   ...
   */
  async sendPaymentApprovedEmail(paymentData: PaymentEmailData): Promise<void> {
    try {
      ...
      if (hasRecentEmail) {
        this.logger.log(
          `E-mail de pagamento aprovado já foi enviado recentemente para inscrição ${paymentData.inscriptionId}. Pulando envio.`,
        );
        return;
      }
      ...
      await this.mailService.sendTemplateMail({
        ...
        subject: `Pagamento Aprovado - ${paymentData.eventName ?? ''}`,
